            ...
            <tbody>
                <?php foreach ($posts['data'] as $post): ?>
                    <tr>
                        ...
                        <td>
                            <a href="/admin/showDraftpost/<?= $post['id'] ?>" class="btn btn-info">Voir</a>
                            <a href="/admin/edit/<?= $post['id'] ?>" class="btn btn-warning">Edit</a>
                            <form action="/admin/publish/<?= $post['id'] ?>" method="POST" style="display:inline-block;">
            ...
            <ul class="pagination">
                <?php for ($i = 1; $i <= $posts['last_page']; $i++): ?>
                    <li class="page-item <?= $i == $posts['page'] ? 'active' : '' ?>">
                        <a class="page-link" href="/admin/listdraftpost?page=<?= $i ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
            </ul>
        </nav>
