<?php $title = htmlspecialchars($posts['category']['name']); ?>
<div class="page-title position-relative">
    <div class="container d-lg-flex justify-content-between align-items-center">
        <h1 class="mb-2 mb-lg-0">Categorie <?= htmlspecialchars($posts['category']['name']) ?> </h1>
        <nav class="breadcrumbs">
            <ol>
                <li><a href="/">Accueil</a></li>
                <li><a href="/category">Catégories</a></li>
                <li class="current"><?= htmlspecialchars($posts['category']['name']) ?></li>
            </ol>
        </nav>
    </div>
</div>
                        ...
                        <ul class="pagination">
                            <?php for ($i = 1; $i <= $posts['last_page']; $i++): ?>
                                <li class="page-item <?= $i == $posts['page'] ? 'active' : '' ?>">
                                    <a class="page-link" href="/category/list/<?= htmlspecialchars($posts['data'][0]['category_id']) ?>?page=<?= $i ?>"><?= $i ?></a>
                                </li>
                            <?php endfor; ?>
                        </ul>
                    </nav>
